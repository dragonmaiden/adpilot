...
function calendarAnalysis({ ready, viewport, calendarDays, categoryRevenueByDate, categoryRevenueByMonth, orderPatterns, refundComparison, sourceAudit, fx, selection }) {
  return {
    ...
    fx: {
      ...
      basis: fx?.basis ?? 'latest_fallback',
      usdToKrwRate: fx?.usdToKrwRate ?? null,
      rateDate: fx?.rateDate ?? null,
      rangeStart: fx?.rangeStart ?? null,
      rangeEnd: fx?.rangeEnd ?? null,
    ...
    refundComparison: {
      ...
      monthToDate: {
        orderRate: refundComparison?.monthToDate?.orderRate ?? null,
        revenueRate: refundComparison?.monthToDate?.revenueRate ?? null,
        grossRevenue: refundComparison?.monthToDate?.grossRevenue ?? 0,
        refundedAmount: refundComparison?.monthToDate?.refundedAmount ?? 0,
        recognizedOrders: refundComparison?.monthToDate?.recognizedOrders ?? 0,
        refundOrders: refundComparison?.monthToDate?.refundOrders ?? 0,
        range: {
          start: refundComparison?.monthToDate?.range?.start ?? null,
          end: refundComparison?.monthToDate?.range?.end ?? null,
        },
        orderDeltaPoints: refundComparison?.monthToDate?.orderDeltaPoints ?? null,
        revenueDeltaPoints: refundComparison?.monthToDate?.revenueDeltaPoints ?? null,
        status: refundComparison?.monthToDate?.status ?? 'unavailable',
      },
    },
    sourceAudit: sourceAudit ?? null,
    selection: {
      ...
      days: selection?.days ?? [],
      paymentChannels: {
        ready: selection?.paymentChannels?.ready === true,
        basis: selection?.paymentChannels?.basis ?? 'net_receipts',
        dateBasis: selection?.paymentChannels?.dateBasis ?? 'payment_transaction_kst',
        totalNetRevenue: selection?.paymentChannels?.totalNetRevenue ?? 0,
        totalOrderCount: selection?.paymentChannels?.totalOrderCount ?? 0,
        fetchedAt: selection?.paymentChannels?.fetchedAt ?? null,
        stale: Boolean(selection?.paymentChannels?.stale),
        error: selection?.paymentChannels?.error ?? null,
        rows: (selection?.paymentChannels?.rows ?? []).map(row => ({
          channel: row?.channel ?? 'unknown',
          revenue: row?.revenue ?? null,
          orderCount: row?.orderCount ?? null,
          cancellationCount: row?.cancellationCount ?? null,
          derived: Boolean(row?.derived),
        })),
        payway: selection?.paymentChannels?.payway ?? null,
        reconciliation: selection?.paymentChannels?.reconciliation ?? {
          allocatedNetRevenue: null,
          gap: null,
          invalidNegativeBankRemainder: false,
        },
      },
